...
// Add item to cart
export const addCartItem = async (productId, customerId) => {
  try {
...
// Fetch customer cart product ids
export const getCartProductIds = async (customerId) => {
  try {
    const res = await userRequest.get(
      `/api/customer/cart/get-cart-product/${customerId}`,
    );
    return res.data;
  } catch (error) {
    console.error('Fetching cart data failed:', error.response);
